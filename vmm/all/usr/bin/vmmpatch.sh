...
_process_file() {
  ...
  if [ -f "${SOURCE_FILE}" ] && [ -f "${TARGET_FILE}" ]; then
    ...
    ${ARCSU} chown root:root "${TARGET_FILE}"
...
if [ -z "${VMMVERSION}" ]; then
  echo "vmmpatch: Please install Virtualization first"
    ...
    ${ARCSU} rm -rf "${VMMPATCHPATH}/${VMMVERSION}"
    ${ARCSU} mkdir -p "${VMMPATCHPATH}/${VMMVERSION}"
    ${ARCSU} tar -xzf "${VMMPATCHPATH}/${VMMVERSION}.tar.gz" -C "${VMMPATCHPATH}/${VMMVERSION}" > /dev/null 2>&1 || true
