...
if [ -z "${VMMVERSION}" ]; then
  echo "vmmpatch: Please install Virtualization first"
    ...
    rm -rf "${VMMPATH}/${VMMVERSION}"
    mkdir -p "${VMMPATH}/${VMMVERSION}"
    tar -xzf "${VMMPATH}/${VMMVERSION}.tar.gz" -C "${VMMPATH}/${VMMVERSION}" > /dev/null 2>&1 || true
    ...
    for F in "${PATCH_FILES[@]}"; do
      SOURCE_FILE="${VMMPATH}/${VMMVERSION}/${F}"
    ...
    if [ "${NEED_PATCH}" = true ]; then
      ...
      /usr/syno/bin/synopkg stop Virtualization > /dev/null 2>&1 || true
      ...
      /usr/syno/bin/synopkg restart Virtualization > /dev/null 2>&1 || true
    else
