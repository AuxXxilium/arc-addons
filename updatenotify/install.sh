...
install_updatenotify() {
  ...
  local DEST="/tmpRoot/usr/lib/systemd/system/arc-updatenotify.service"
  {
    echo "[Unit]"
...
case "${1}" in
  late)
    install_updatenotify "${1}"
    ;;
  uninstall)
    uninstall_updatenotify "${1}"
    ;;
  *)
