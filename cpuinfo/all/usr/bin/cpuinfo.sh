...
COREC="Cores: $(grep -c 'cpu cores' /proc/cpuinfo 2>/dev/null)"                                            # str
THREADC="Threads: $(grep -c 'siblings' /proc/cpuinfo 2>/dev/null)"                                         # str
SPEED="$(grep -m1 'MHz' /proc/cpuinfo 2>/dev/null | cut -d: -f2 | cut -d. -f1 | xargs)"                    # int
GOVERNOR="Governor: $(grep -o 'governor=[^ ]*' /proc/cmdline 2>/dev/null | cut -d'=' -f2 | xargs)"         # str
CORES="${COREC:-1} | ${THREADC:-1} | ${GOVERNOR:-none}"                                                    # str
SPEED="${SPEED:-0}"                                                                                        # int
