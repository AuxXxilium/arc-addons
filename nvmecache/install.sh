...
local MODELS="DS918+ RS1619xs+ DS1019+ DS718+ DS1621xs+"
local MODEL="$(cat /proc/sys/kernel/syno_hw_version)"
...
if [ "${1}" = "patches" ]; then
  ...
  local BOOTDISK_PART3_PATH BOOTDISK_PART3_MAJORMINOR BOOTDISK_PART3 BOOTDISK BOOTDISK_PHYSDEVPATH
  ...
  local SO_FILE="/tmpRoot/usr/lib/libsynonvme.so.1"
