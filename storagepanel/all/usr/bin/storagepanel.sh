...
_UNIQUE="$(${ARCSU} get_key_value /etc.defaults/synoinfo.conf unique)"
_BUILD="$(${ARCSU} get_key_value /etc.defaults/VERSION buildnumber)"
...
FILE_GZ="${FILE_JS}.gz"
[ -f "${FILE_JS}" ] && [ ! -f "${FILE_GZ}" ] && ${ARCSU} gzip -c "${FILE_JS}" >"${FILE_GZ}"
...
if [ "${1}" = "-r" ]; then
  if [ -f "${FILE_GZ}.bak" ]; then
    ${ARCSU} mv -f "${FILE_GZ}.bak" "${FILE_GZ}"
    ${ARCSU} gzip -dc "${FILE_GZ}" >"${FILE_JS}"
  fi
  exit
fi
...
if [ -z "${HDD_BAY}" ]; then
  IDX="$(${ARCSU} synodisk --enum -t internal 2>/dev/null | grep "Disk id:" | tail -n1 | cut -d: -f2 | xargs)"
  IDX=${IDX:-0}
  while [ ${IDX} -le 60 ]; do
...
if [ -z "${SSD_BAY}" ]; then
  IDX="$(${ARCSU} synodisk --enum -t cache 2>/dev/null | grep "Disk id:" | tail -n1 | cut -d: -f2 | xargs)"
  SSD_BAY="$((${IDX:-0} / 8 + 1))X8"
fi

[ ! -f "${FILE_GZ}.bak" ] && ${ARCSU} cp -pf "${FILE_GZ}" "${FILE_GZ}.bak"
...
${ARCSU} sed -i "s/\"${_UNIQUE}\",//g; s/,\"${_UNIQUE}\"//g; s/${HDD_BAY}:\[\"/${HDD_BAY}:\[\"${_UNIQUE}\",\"/g; s/M2X1:\[\"/M2X1:\[\"${_UNIQUE}\",\"/g; s/${OLD}/${NEW}/g" "${FILE_JS}"
${ARCSU} gzip -c "${FILE_JS}" >"${FILE_GZ}"
